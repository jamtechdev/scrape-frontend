...
export default function LoginForm({ onSubmit, loading, onForgotPassword, onGoogleLogin }) {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // Check if user already has valid token - skip OAuth and redirect to dashboard
      if (response.code === 200 && response.data?.skipOAuth && response.data?.hasValidToken) {
      ...
      alert('Error: ' + (err.message || 'Failed to initiate Google login'));
      setGoogleLoading(false);
    }
  };
