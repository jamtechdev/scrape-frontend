...
export default function History() {
  ...
  const [loadingJobs, setLoadingJobs] = useState({}); // Track which jobs are being paused/resumed
                              ...
                              <div className="flex items-center gap-2">
                                {(canPauseJob(job) || canResumeJob(job)) && (
                                  <button
                                    ...
                                    className={`px-4 py-2 text-white rounded-lg transition text-sm font-medium flex items-center gap-2 ${
                                      canPauseJob(job)
                                        ? 'bg-orange-600 hover:bg-orange-700'
                                        : 'bg-blue-600 hover:bg-blue-700'
                                    } ${loadingJobs[job.id] ? 'opacity-70 cursor-not-allowed' : ''}`}
                                  >
                                    {loadingJobs[job.id] && (
                        ...
                        <div className="mt-4 space-y-2">
                          {(canPauseJob(job) || canResumeJob(job)) && (
                            <button
                              onClick={() => handleTogglePauseResume(job)}
                              disabled={loadingJobs[job.id]}
                              className={`w-full px-4 py-2.5 text-white rounded-lg transition text-sm font-medium flex items-center justify-center gap-2 ${
                                canPauseJob(job)
                                  ? 'bg-orange-600 hover:bg-orange-700'
                                  : 'bg-blue-600 hover:bg-blue-700'
                              } ${loadingJobs[job.id] ? 'opacity-70 cursor-not-allowed' : ''}`}
